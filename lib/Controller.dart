...
class Controller {
  ...
  Future<void> AddUtente(String nome, String cognome, String data, String sesso,
      String provincia, String citta, String email, String user,
    ...
    database.addUser(new Utente(nome, cognome, data, sesso, idLuogo, email, user, password));
  }


  Future<void>AddAzienda(String nome_azienda,String email,String citta,String provincia,String partita_iva,
  String tipologia_azienda,String password,String user) async {
    bool celuogo=false;//per valutare se il luogo è gia stato isneirto
    int idLuogo; //id luogo da associare alla persona
    int idIndirizzo;
    List <Luogo> list= await database.getLuoghiList();
    ...
    idIndirizzo=await database.addIndirizzo(new Indirizzo(idLuogo, null, null));
    int idazienda=await database.addAzienda(new Azienda(nome_azienda, idIndirizzo, partita_iva,tipologia_azienda,
        email, user, password));
    for(int i=4;i>0;i--){
  ...
  Future<bool> getChat(String id_utente,List <String> listchat) async {
    if(listchat.isEmpty) {
      print("entro");
      List<QueryDocumentSnapshot> list = await database.getTable(
          "Chat" + id_utente);
      list.forEach((element) {
  ...
  Future<void> UpdateUser(String nome,String cognome,String username,String email, String cellulare,File foto_profilo) async {

        if(foto_profilo!= null)
          {
            String fileName = Path.basename(foto_profilo.path);
            fileName=database.currentUser.id.toString()+ fileName;//compone id+nomefotocarticata//
             await database.AddFoto(fileName, foto_profilo);
            database.currentUser.foto_profilo= await database.getPathFotoProfilo(fileName);
          }
  ...
  Future<void> UpdateAzienda(  File foto_profilo,File foto_copertina,File foto_hashtag_1,File foto_hashtag_2,
  File foto_hashtag_3, File foto_hashtag_4,String indirizzo, String descrizione, String nome,
  String nome_hash_1,String nome_hash_2,String nome_hash_3,String nome_hash_4)async{
    if(foto_profilo!= null)
    {
      String fileName = Path.basename(foto_profilo.path);
      fileName=database.currentAzienda.id.toString()+ fileName;//compone id+nomefotocarticata//
      await database.AddFoto(fileName, foto_profilo);
  ...
  Future<void> ThereIsCall() async { //cerca se l'azienda corrente sta venendo chiamata
    if(database.isAzienda){//si attiva solo se è un azienda
        ...
        if(current_chiamata.id_azienda==database.currentAzienda.id){
          ...
          listuser.forEach((current_user) async {
            if(current_chiamata.id_utente==current_user.id){
              ...
              await database.RemoveCall(new Chiamata(current_chiamata.id_azienda, current_user.id, 7, 7));

              Route route = MaterialPageRoute(builder: (context) => Call(channelName:
              "ChiamataN"+ current_chiamata.id_azienda.toString()+ current_user.id.toString() //da nome canale univoco
                  ,user:current_user));
  ...
  Future<void> ChiamaAzienda(int id_azienda,BuildContext context) async {
    ...
    await database.addChiamata(
        new Chiamata(id_azienda, database.currentUser.id, 10, 11));
    Route route = MaterialPageRoute(builder: (context) => Call(channelName:
    "ChiamataN"+ id_azienda.toString()+ database.currentUser.id.toString() ,azienda: azienda_chiamata,//da nome canale univoco
    ));
  ...
  Future <void> LanciaOfferta( String titolo, String prezzo, String opzioneRitiro,Utente user) async {
    await database.addOfferta(
        new Offerta ( titolo, int.parse(prezzo), '', database.currentAzienda.id, user.id, opzioneRitiro));
  print("added Offerta");
  ...
  Future<void> ThereIsOfferta() async {
    ...
    if(!database.isAzienda){//si attiva solo se non è un azienda
